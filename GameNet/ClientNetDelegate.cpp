...
, m_iSendCout(0)
{
    ...
    cocos2d::Director::getInstance()->getScheduler()->schedule(CC_SCHEDULE_SELECTOR(ClientNetDelegate::runSchedule),this,0.0f,false);
}
...
ClientNetDelegate::~ClientNetDelegate()
{
    ...
    m_oSocket.ccClose();
    cocos2d::Director::getInstance()->getScheduler()->unschedule(CC_SCHEDULE_SELECTOR(ClientNetDelegate::runSchedule),this);
}


void ClientNetDelegate::setInetAddress(const ClientInetAddress& oInetAddress)
{
    m_oInetAddress = oInetAddress;
}

const ClientInetAddress& ClientNetDelegate::getInetAddress() const
{
    return m_oInetAddress;
}

void ClientNetDelegate::setSoTimeout(float fSoTimeout)
{
    m_fSoTimeout = fSoTimeout;
}

float ClientNetDelegate::getSoTimeout() const
{
    return m_fSoTimeout;
}

void ClientNetDelegate::send(const char* pData,int iSize)
{
...
bool ClientNetDelegate::isConnected()
{
    return m_eStatus == eSocketConnected;
}

bool ClientNetDelegate::connect()
{
    m_iSendCout = 0;
...
void ClientNetDelegate::disconnect()
{
...
void ClientNetDelegate::runSchedule(float dt)
{
    switch( m_eStatus )
    {
...
bool ClientNetDelegate::runRead()
{
...
bool ClientNetDelegate::runWrite()
{
